...
function MediaUI(social) {
    ...
    this.init = function() {
        ...
        loadData(function() {
			social.getAlbumsByGroup(viewer.id, "FRIENDS", function(response) {
    ...
    function renderAlbums(albums) {
        ...
        dojo.create('td', {innerHTML: viewer.name.formatted + "'s Friends' Albums", className: 'albumsTitle'}, tr);
        ...
        function onClickAlbum(userId, album) {
            social.getMediaItemsByAlbum(album.ownerId, album.id, function(response) {
    ...
    function renderAlbumsByUser(userId, callback) {
        social.getAlbumsByGroup(userId, "FRIENDS", function(response) {
            renderAlbums(response.list);
            divManager.showAlbums();
